...
function startWebar(){
  ...
  HandTrackerThreeHelper.init({
    ...
    NNsPaths: ['util/neuralNets/NN_FOOT_' + _settings.NNVersion.toString() + '.json'],
    maxHandsDetected: 2,
    ...
    console.log('INFO in startWebar.js: an error happens ', err);
  });
} 
...
function start(three){
  three.loadingManager.onLoad = function(){
    console.log('INFO in startWebar.js: Everything is loaded');
...
if (document.readyState === 'complete' || document.readyState === 'interactive') {
    // The page is ready, run immediately
    startWebar();
} else {
    // Fallback: If it's somehow not ready, wait for DOMContentLoaded
    document.addEventListener('DOMContentLoaded', startWebar);
}